module.exports = async function bundle (s, filename, { builtins, header = '', footer = '' } = {}) {
  ...
  if (cjs.length) {
    ...
    for (let i = cjs.length - 1; i >= 0; i--) {
      ...
      const e = isBuiltin ? builtins + '.get(' + JSON.stringify(s.mapPath(cjs[i].filename)) + ')' : '{}'
    ...
    for (let i = cjs.length - 1; i >= 0; i--) {
      ...
      out += 'require.define(' + JSON.stringify(s.mapPath(mod.filename)) + ', ' + JSON.stringify(s.mapPath(mod.dirname)) + ', ' + JSON.stringify(map) + ', function '
  ...
  if (main.type === 'module') {
    ...
    out += 'require(' + JSON.stringify(s.mapPath(main.filename)) + ')\n'
  }
