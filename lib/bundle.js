module.exports = async function bundle (s, filename, { builtins, header = '', footer = '' } = {}) {
  ...
  if (cjs.length) {
    ...
    for (let i = cjs.length - 1; i >= 0; i--) {
      const isBuiltin = !!(cjs[i].builtin && builtins)
      const e = isBuiltin ? builtins + '.get(' + JSON.stringify(cjs[i].filename) + ')' : '{}'
      out += '  ' + JSON.stringify(cjs[i].filename) + ': { exports: ' + e + ', bootstrap: null, require: null, filename: null, dirname: null }' + (i === 0 ? '' : ',') + '\n'
    }
    out += '}\n'
    out += 'require.define = function (filename, dirname, map, fn) {\n'
    out += '  scopedRequire.cache = require.cache\n'
    out += '  const mod = require.cache[filename]\n'
    ...
    for (let i = cjs.length - 1; i >= 0; i--) {
      ...
      out += 'require.define(' + JSON.stringify(mod.filename) + ', ' + JSON.stringify(mod.dirname) + ', ' + JSON.stringify(map) + ', function '
    ...
    out += '  const mod = require.cache[req]\n'
  ...
  if (main.type === 'module') {
    ...
    out += 'require(' + JSON.stringify(main.filename) + ')\n'
  }
