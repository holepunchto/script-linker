module.exports = async function bundle(
  s,
  filename,
  { require = true, builtins, header = '', footer = '' } = {}
) {
  ...
  for await (const { isImport, module } of s.dependencies(filename)) {
    ...
    if (isImport) {
      if (esm) esm += ',\n'
      esm +=
        '    ' + JSON.stringify('bundle:' + s.mapPath(module.filename)) + ': '
      esm += JSON.stringify(
        'data:application/javascript;base64,' +
          Buffer.from(module.toESM()).toString('base64')
      )
    }
  }
  ...
  if (cjs.length) {
    ...
    for (let i = cjs.length - 1; i >= 0; i--) {
      ...
      out +=
        '  ' +
        JSON.stringify(s.mapPath(cjs[i].filename)) +
        ': { exports: ' +
        e +
        ', bootstrap: null, require: null, filename: null, dirname: null }' +
        (i === 0 ? '' : ',') +
        '\n'
    }
    out += '}\n'
    ...
    out += '  scopedRequire.addon = scopedAddon\n'
    ...
    out +=
      '    if (globalRequire.cache.hasOwnProperty(req)) return globalRequire(req)\n'
    out +=
      '    if (!map.hasOwnProperty(req)) throw new Error("Cannot require \'" + req + "\'")\n'
    out += '    return globalRequire(map[req])\n'
    out += '  }\n'
    out += '  function scopedAddon (req = dirname) {\n'
    out +=
      '    if (typeof require !== "function" || !require.addon) throw new Error("Addons not supported")\n'
    out += '    return require.addon(req)\n'
    out += '  }\n'
    out += '}\n'
    for (let i = cjs.length - 1; i >= 0; i--) {
      const mod = cjs[i]
      ...
      out +=
        ...
        JSON.stringify(s.mapPath(mod.filename)) +
        ', ' +
        JSON.stringify(s.mapPath(mod.dirname)) +
        ', ' +
        JSON.stringify(map) +
        ', function '
      out += '(require, module, exports, __filename, __dirname) {\n'
      out +=
        mod.type === 'json' ? 'module.exports = ' + mod.source : mod.toCJS()
      out += '})\n'
    }
    out += 'function globalRequire (req) {\n'
    out +=
      '  if (!globalRequire.cache.hasOwnProperty(req)) throw new Error("Cannot require \'" + req + "\'")\n'
    out += '  const mod = globalRequire.cache[req]\n'
    out += '  if (mod.bootstrap !== null) {\n'
    out += '    const bootstrap = mod.bootstrap\n'
    out += '    mod.bootstrap = null\n'
    out +=
      '    bootstrap(mod.require, mod, mod.exports, mod.filename, mod.dirname)\n'
    out += '  }\n'
  ...
  if (main.type === 'module') {
    out +=
      'import(' + JSON.stringify('bundle:' + s.mapPath(main.filename)) + ')\n'
  } else {
  ...
  function getBuiltin(name) {
    return require
      ...
      : builtins + '.get(' + JSON.stringify(name) + ')'
  }
}
