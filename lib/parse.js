...
exports.exports = function exports (src, type, opts) {
  if (type === 'module') return formatESMExports(mjs.parse(src)[1])

  let named = type === 'json' ? [] : cjs.parse(src).exports
  if (opts && opts.defines) named = addDefines(named, src)
...
// sniffs for Object.defineProperty(exports) also, used by some packages, but unsure the
// perf impact here...
function addDefines (list, src) {
  const originalLength = list.length
  for (const m of src.match(/Object\.defineProperty\(\s*exports,\s*["']([^'"]+)['"]/g) || []) {
    list.push(m.match(/["']([^'"]+)['"]/)[1])
  }
  if (list.length === originalLength) return list
  return [...new Set(list)]
}
