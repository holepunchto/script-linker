...
module.exports = class Module {
  ...
  toESM () {
    ...
    out += 'const __mod__ = ' + requireFromSource(this.linker._ns, this.filename, this.toCJS()) + '\n'

    if (e) {
      for (const key of e.named) {
        if (key === 'default') continue
        if (!isProperty(key)) continue
        out += `export const ${key} = typeof __mod__.${key} === 'function' ? __mod__.${key}.bind(__mod__) : __mod__.${key}\n`
      }
      if (e.default) {
        out += 'export default (typeof __mod__ === \'object\' && __mod__ && __mod__.__esModule && __mod__.default !== undefined)'
        out += ' ? (typeof __mod__.default === \'function\' ? __mod__.default.bind(__mod__) : __mod__.default)'
        out += ' : __mod__\n'
      }
    }
  ...
  _transform (transforms) {
    ...
    if (resolutions) {
      ...
      for (const r of resolutions) {
        ...
        if (r.input) { // import "string"
          ...
          t = createImportString(this.linker._ns, this.filename)
        }
    ...
    if (this.type !== 'json') {
      out += (out.endsWith('\n') ? '' : '\n') + '//# sourceURL='
      if (needsSourceMap) out += this.filename + '+esm-transformed' // hack to make source maps look correct url wise
  ...
  async _refresh () {
    ...
    const info = (stat && stat.type)
      ? { type: stat.type, filename: null, package: null, malformed: false }
  ...
  _wrapBuiltin (mod) {
    let src = `const __mod__ = ${this.linker._ns}require(${JSON.stringify(this.filename)})\n`
...
function requireFromSource (ns, filename, src) {
  return ns
    ? `${ns}requireFromSource(${JSON.stringify(filename)}, ${JSON.stringify(src)})`
