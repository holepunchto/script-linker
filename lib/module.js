...
const unixify = require('./unixify.js')

module.exports = class JSModule {
  constructor (linker, filename, { defaultType = linker.defaultType } = {}) {
    this.linker = linker
    this.filename = unixify(filename)
    this.dirname = unixify(path.dirname(filename))
  ...
  async updateResolutions (resolutions = this.resolutions) {
    ...
    for (let i = 0; i < outs.length; i++) {
      const output = outs[i].status === 'fulfilled' ? outs[i].value : null
      resolutions[i].output = (output) ? unixify(output) : output
    }
  }
...
function isCustomScheme (str) {
  // returns true if custom url scheme (i.e. protocol) except windows style drive, e.g. "C:"
  // See, https://url.spec.whatwg.org/#url-miscellaneous for details
  try {
    const { protocol } = new URL(str) // throws if invalid url, e.g. /scheme://
    return (protocol && !(/^[a-z]:$/i).test(protocol))
  } catch { return false }
}
