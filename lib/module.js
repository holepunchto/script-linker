...
module.exports = class Module {
  ...
  toESM () {
    ...
    out += `const mod = global[Symbol.for('scriptlinker')].requireFromSource(${JSON.stringify(this.filename)}, ${JSON.stringify(this.toCJS())})\n`
  ...
  _transform (transforms) {
      ...
      for (const r of resolutions) {
        ...
        if (r.input) { // import "string"
          if (r.output) {
            ...
            const o = this.linker._mapImport(r.output)
    ...
    if (this.linker.linkSourceMaps && transformed) {
