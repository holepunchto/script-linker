...
module.exports = class Module {
  ...
  get exports () {
    return this._module && this._module.exports
  }
  ...
  getWarmup (ack) {
    return { filename: this.filename, ack: ack ? this._clock : 0, source: this.toCJS() }
  }

  setWarmup ({ ack, exports }) {
    if (this._module && ack === this._clock) {
      this._module.exports = { default: true, named: exports }
    }
  }
  ...
  async warmup () {
    ...
    if (mod.type !== 'commonjs') return
    if (!this.linker._userWarmup) return

    const w = await this.linker._userWarmup([this.getWarmup(true)])
    if (w && w.length) this.setWarmup(w[0])
  }
  ...
  async _refresh () {
    ...
    if (this._module !== mod) {
      this._clock = ++this.linker._clock
    }
