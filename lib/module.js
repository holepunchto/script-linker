        ...
        if (r.input) {
          if (r.isImport) { // import "string"
            if (r.output) {
              ...
              t = 'throw ' + errorString(r.input, this.filename)
            }
          } else { // require("string")
            const e = r.output ? 'null' : errorString(r.input, this.filename)
            start = s
            end = e
            t = JSON.stringify(r.output) + ', { resolved: ' + !!r.output + ', error: ' + e + '}'
          }
        } else {
...
function compareSourceNodes (a, b) {
...
function toBase64 (s) {
  return b4a.toString(b4a.from(s), 'base64')
}

function errorString (input, filename) {
  return `new Error(${JSON.stringify(`Cannot find package '${input}' imported from ${filename})`)})`
}
