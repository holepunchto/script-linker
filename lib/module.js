...
module.exports = class Module {
  constructor (linker, filename, { defaultType = linker.defaultType, transformSource = (src) => src } = {}) {
    this.linker = linker
    ...
    this._transformSource = transformSource
  ...
  async loadPackage () {
    const info = this._moduleInfo

    if (!info) return null
    if (info.package) return info.package

    const pkg = await this.linker.resolvePackageJSON(this.filename)
    if (!pkg) return null

    if (!info.package) info.package = JSON.parse(await this.linker.drive.get(pkg))
    return info.package
  }
  ...
  async _refresh () {
    ...
    const source = this._transformSource(b4a.toString(await this.linker._readFile(node, true)))
