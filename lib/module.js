...
module.exports = class JSModule {
  constructor (linker, filename, { defaultType = linker.defaultType } = {}) {
    this.linker = linker
    this.isLinkerBuiltin = this.linker.builtins.has(filename)
    this.filename = (this.isLinkerBuiltin) ? filename : unixresolve(filename)
    this.dirname = (this.isLinkerBuiltin) ? '/' : unixresolve(this.filename, '..')
