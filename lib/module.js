...
module.exports = class Module {
  ...
  get rexports() {
    ...
    for (const n of this.namedImports) {
      if (n.isExport && n.isWildcard && n.from.output)
        this._reexports.push(n.from.output)
    }
  ...
  toCJS() {
    ...
    if (this._module.type === 'module')
      throw new Error('Cannot convert an ESM module to CommmonJS')
  ...
  toESM() {
    ...
    const req =
      this.linker._ns + 'require(' + JSON.stringify(f) + ', { resolved: true })'
    ...
    out +=
      "export default (typeof __mod__ === 'object' && __mod__ && __mod__.__esModule && __mod__.default !== undefined)"
    out += ' ? __mod__.default'
    out += ' : __mod__\n'
  ...
  _transform(transforms) {
    ...
    if (resolutions) {
      ...
      if (requireMap) {
        out +=
          '/* @scriptlinker-resolutions ' + JSON.stringify(requireMap) + ' */\n'
      }
      ...
      for (const r of resolutions) {
        ...
        if (r.input) {
          ...
          t = createImportString(
            this.linker._ns,
            this.linker.mapPath(this.filename)
          )
        }
        ...
        if (transforms)
          transforms.push({
            original: r.position,
            generated: [out.length, out.length + t.length]
          })
        out += t
      }
    }
    ...
    const needsSourceMap =
      this.linker.linkSourceMaps && transformed && !this.linker.bare

    if (this.type !== 'json') {
      out += (out.endsWith('\n') ? '' : '\n') + '//# sourceURL='
      if (needsSourceMap)
        out += this.linker.mapPath(this.filename) + '+esm-transformed' // hack to make source maps look correct url wise
  ...
  async _refresh() {
    ...
    const mod =
      cache && cache.resolutions
        ? {
            type: info.type,
            resolutions: cache.resolutions,
            namedImports: [],
            exports: cache.exports || null
          }
  ...
  async _getModuleInfo() {
    ...
    if (info.package)
      info.type =
        info.package.type === 'module'
          ? 'module'
          : info.package.type === 'commonjs'
            ? 'commonjs'
            : null
  ...
  async _updateResolutions(resolutions) {
    ...
    for (const res of resolutions) {
      p.push(
        res.input
          ? this.resolve(res.input, { isImport: res.isImport })
          : Promise.resolve(null)
      )
    }
    const outs = await Promise.allSettled(p)
    for (let i = 0; i < outs.length; i++) {
      resolutions[i].output =
        outs[i].status === 'fulfilled' ? outs[i].value : null
    }
  }
  ...
  _generateSourceMap() {
    ...
    acorn.parse(source, {
      ...
      onToken(t) {
        ...
        if (cur && cur.generated[0] <= t.start && t.end <= cur.generated[1]) {
          if (!firstTransformed) {
            ...
            nextDelta =
              cur.generated[1] -
              cur.generated[0] -
              (cur.original[1] - cur.original[0])
          }
          push(
            firstTransformed.loc.start.line,
            firstTransformed.loc.start.column,
            t.loc.start.line,
            t.loc.start.column
          )
          return
        ...
        push(
          t.loc.start.line,
          t.loc.start.column - columnDelta,
          t.loc.start.line,
          t.loc.start.column
        )
      }
...
function createErrorModule(input, filename) {
  const src =
    'throw new Error(' +
    JSON.stringify(`Cannot find package '${input}' imported from ${filename}`) +
    ')'
